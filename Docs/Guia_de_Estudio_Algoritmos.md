...
Esta guía acompaña la presentación y el documento teórico del proyecto **ANS‑STC**. Aquí no
...
> **Una advertencia sobre el nombre.** El proyecto se llama **ANS‑STC** por sus raíces
> teóricas (dos técnicas llamadas *ANS* y *STC*). Pero el sistema que realmente se construyó y
> funciona es **una marca de agua robusta** que usa otras herramientas (Brotli e ISS). A ANS y
> STC les dedicamos una sección de *contexto*, porque explican de dónde viene la idea, pero
> **no forman parte del sistema final**.
...
# Parte 3 · Contexto: la herencia «ANS‑STC»

> Esta sección es de **cultura general del proyecto**: explica de dónde viene el nombre y la
> idea rectora. **El sistema final no usa estas técnicas** (usa Brotli e ISS), pero entenderlas
> ayuda a apreciar por qué tomamos las decisiones que tomamos.
...
Es exactamente invertible, así que se puede decodificar sin pérdida. Le da la mitad del nombre
al proyecto («ANS»). En la práctica, **Brotli** cumple el mismo papel de comprimir.

## 3.2 STC (Códigos de Síndrome‑Trellis) y el algoritmo de Viterbi

🎯 **La idea.** Es la otra mitad del nombre («STC»). Es una forma *óptima* de esconder muchos
...
(`Docs/Fundamentos_Teoricos_ANS-STC.tex`).*
